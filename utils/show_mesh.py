...
def plotly_trisurf(x, y, z, colors, simplices, colormap=cm.RdBu, plot_edges=False):
    ...
    if plot_edges is False:  # the triangle sides are not plotted
        ...
        Xe, Ye, Ze = [reduce(lambda x, y: x+y, lists_coord[k],
                             lists_coord[0]) for k in range(3)]
        lines = Scatter3d(x=Xe, y=Ye, z=Ze, mode='lines',
...
def textured_mesh(mesh, per_vertex_signal=None, samples=None, filename='visualize mesh'):
    ...
    x = mesh.vertices.transpose()[0]
    y = mesh.vertices.transpose()[1]
    z = mesh.vertices.transpose()[2]
    ran = mesh.bounds[1, :] - mesh.bounds[0, :]
    data1 = plotly_trisurf(x, y, z, per_vertex_signal,
                           mesh.faces, colormap=cm.RdBu, plot_edges=True)
